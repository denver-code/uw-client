...
export class ProviderChatComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.sessionService.IsAuthenticatedPage('ProviderChats')) {
      this.StartPage();
    } else {
  ...
  public LoadData(pageSize: number, currentPageNumber: number) {
    ...
    this.service.httpPost(`notification/provider_list_chats`, {page: currentPageNumber , amount: this.pageSize}).subscribe(
      data => {
        // Success
